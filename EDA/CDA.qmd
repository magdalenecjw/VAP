...
plot <- touristdata_clean %>% 
  select(c(7,8,9,21,22,23,24,28,29,30)) %>% 
  gather() %>%
  ggplot(aes(value)) +
...
```{r}
plot_map_eda(metric = "total_cost", style = "jenks", classes = 6, minvisitors = 0)
```
...
```
Majority of big spender indeed comes from Australia, followed by US, Canada, New Zealand, Denmark, and South Africa.
However, it is noted that there is no statistically significant difference among many of them
